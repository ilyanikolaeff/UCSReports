    ...
    class ListOfAlgorithmsViewModel : ViewModelBase, IReportViewModel
    {
                ...
                () =>
                {
                    try
                    {
                        if (SelectedAlgorithm != null)
                        {
                            ...
                            mvvmProvider.ShowView(currentVM);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private IReportViewModel GetViewModel()
        {
            if (SelectedAlgorithm.AlgorithmType == AlgorithmType.Emergency)
            {
                ...
                emergencyAlgViewModel.CurrentAlgorithm.Steps = new List<Step>(emergencyAlgModel.GetEmergencySteps(SelectedAlgorithm, TZName));
            ...
            else if (SelectedAlgorithm.AlgorithmType == AlgorithmType.Regular)
            {
                ...
                regularAlgViewModel.CurrentAlgorithm.Steps = new List<Step>(regularAlgModel.GetNormalSteps(SelectedAlgorithm, TZName));
