...
namespace UCSReports
{
    public class Settings
    {
        ...
        public HdaConnection HdaConnectionSettings { get; set; }
        public int IntervalChange { get; private set; } = 4;
        private Settings()
        {
            LoadSettings();
        }
        public void LoadSettings()
        {
            ...
            // hda conn settings
            HdaConnectionSettings = new HdaConnection
            {
                IPAddress = xRoot.Element("HDAServer").Attribute("IP").Value,
                ServerName = xRoot.Element("HDAServer").Attribute("Name").Value
            };
            ...
            foreach (var xZone in xZones)
            {
                ...
                if (xZone.Attribute("Protections")?.Value != null)
                {
                    currentSettings.IsExistProtSettings = true;
                    
                    var xProtections = XDocument.Load(CheckPath(xZone.Attribute("Protections").Value)).Root;

                    // 1 level
                    foreach (var xProt in xProtections.Element("FirstLevel").Element("Protections").Elements("Protection"))
                    {
                        currentSettings.FirstLevelProtection.Add(xProt.Attribute("Tag").Value, xProt.Attribute("Name").Value);
                    }
                    // 2 level
                    foreach (var xProt in xProtections.Element("SecondLevel").Element("Flags").Elements("Flag"))
                    {
                        var currentFlag = new Flag
                        {
                            Number = int.Parse(xProt.Attribute("Num").Value)
                        };
                        // bits
                        foreach (var xBit in xProt.Elements("Bit"))
                        {
                            currentFlag.Bits.Add(int.Parse(xBit.Attribute("Num").Value), xBit.Attribute("Name").Value);
                        }
                        currentSettings.Flags.Add(currentFlag);
                    }
                }
