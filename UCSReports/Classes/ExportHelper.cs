        ...
        public void ExportProtections(IEnumerable<string> data, IEnumerable<Protection> protections, out string savedFileName)
        {
            ...
            try
            {
                ...
                foreach (var prot in protections)
                {
                    ...
                    // name
                    worksheet.Columns["B"][rowNumber].Value = prot.Name;
                    // activation time
                    worksheet.Columns["C"][rowNumber].Value = $"'{prot.ActivationTime}";
                    // deactivation time
                    //worksheet.Columns["D"][rowNumber].Value = $"'{prot.DeactivationTime}";
                    // duration
                    //worksheet.Columns["E"][rowNumber].Value = $"'{prot.Duration}";

                    worksheet.Range[$"A{rowNumber + 1}:C{rowNumber + 1}"].Borders.SetAllBorders(Color.Black, BorderLineStyle.Thin);
        ...
        public void ExportMasks(IEnumerable<string> data, IEnumerable<Mask> masks, out string savedFileName)
        {
            ...
            try
            {
                ...
                foreach (var mask in masks)
                {
                    ...
                    worksheet.Columns["A"][rowNumber].Value = index;
                    // name
                    worksheet.Columns["B"][rowNumber].Value = mask.Name;
                    // activation time
                    worksheet.Columns["C"][rowNumber].Value = $"'{mask.TimeSet}";
                    // deactivation time
                    worksheet.Columns["D"][rowNumber].Value = $"'{mask.TimeUnset}";
                    // duration
                    worksheet.Columns["E"][rowNumber].Value = $"'{mask.Duration}";
                    // tor flag
                    worksheet.Columns["F"][rowNumber].Value = $"'{mask.IsTorMask}";

                    worksheet.Range[$"A{rowNumber + 1}:F{rowNumber + 1}"].Borders.SetAllBorders(Color.Black, BorderLineStyle.Thin);
